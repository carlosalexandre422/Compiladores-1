...
impl<'a> Parser<'a> {
    ...
    fn parse_expr(&mut self) -> Result<Expr, String> {
        let mut expr = self.parse_exp_a()?;  // Trata soma e subtração
    
        // Aqui trata os operadores de comparação
        while let Some(c) = self.peek() {
            let op = match c {
                '=' => {
                    self.next();
                ...
                _ => break,  // Quebra se não for um operador de comparação
            };
    
            let right = self.parse_exp_a()?;  // Trata a próxima expressão (soma, subtração, etc.)
            expr = Expr::OpBin {
                operador: op,
                esq: Box::new(expr),
                dir: Box::new(right),
