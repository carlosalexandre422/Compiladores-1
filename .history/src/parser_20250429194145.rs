...
impl<'a> Parser<'a> {
    ...
    fn parse_expr(&mut self) -> Result<Expr, String> {
        let mut expr = self.parse_exp_a()?; // Parse as expressões aritméticas (+, -, *, /)
    
        // Agora vamos lidar com as comparações, que têm menor precedência que as operações aritméticas
        while let Some(c) = self.peek() {
            let op = match c {
                '=' => {
                    self.next();
                ...
                _ => break,  // Se não for um operador de comparação, para a iteração
            };
    
            let right = self.parse_exp_a()?; // Faz a mesma coisa para a expressão à direita do operador de comparação
            expr = Expr::OpBin {
                operador: op,
                esq: Box::new(expr),
                dir: Box::new(right),
