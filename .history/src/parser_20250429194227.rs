...
impl<'a> Parser<'a> {
    ...
    fn parse_cmd(&mut self) -> Result<Cmd, String> {
        if self.match_kw("if")? {
            let cond = self.parse_expr()?; // ← corrige a posição do `parse_expr`
    ...
    fn parse_expr(&mut self) -> Result<Expr, String> {
        let mut expr = self.parse_exp_a()?; // Parse operações aritméticas (+, -, *, /)
    
        // Agora lidamos com os operadores de comparação
        // Eles devem vir depois de expressões aritméticas
        while let Some(c) = self.peek() {
            let op = match c {
                '=' => {
                    self.next();
                ...
                '<' | '>' => {
                    self.next().unwrap().to_string() // Processa '<' ou '>'
                }
                _ => break,  // Se não for um operador de comparação, pare
            };
    
            // A parte à direita da comparação também será uma expressão aritmética
            let right = self.parse_exp_a()?; // Aqui, usamos `parse_exp_a` novamente para garantir que a direita também seja uma expressão válida
            expr = Expr::OpBin {
                operador: op,
                esq: Box::new(expr),
                dir: Box::new(right),
    ...
    fn parse_kw(&mut self, kw: &str) -> Result<bool, String> {
        ...
        // Verifica se o próximo caractere é um separador (não alfanumérico)
